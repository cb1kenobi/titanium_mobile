				...
				switch (prop) {
					...
					case 'firefoxos':
						dom.create(prop, null, node, function (section) {
							Object.keys(value[prop]).forEach(function (key) {
								dom.create(key, {
									nodeValue: key == 'manifest-json' ? JSON.stringify(value[prop][key], null, '\t').split('\n').join('\n\t\t\t') : value[prop][key]
								}, section);
							});
						});
						break;
					...
					xml.forEachElement(node, function (elem) {
						switch (elem.tagName) {
							...
							case 'firefoxos':
								mobileweb[elem.tagName] = {};
								xml.forEachElement(elem, function (subelem) {
									mobileweb[elem.tagName][subelem.tagName] = subelem.tagName == 'manifest-json' ? eval('(' + xml.getValue(subelem) + ')') : xml.getValue(subelem);
								});
								break;
